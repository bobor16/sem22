...
«««	«IF component.devices.exists[e|e.deviceType instanceof MoistureSensor]»
«««	#define SensorPin 34
«««	float sensorValue=0;
«««	«ENDIF»
...
uint8_t broadcastAddress[] = {«component.byte01», «component.byte02»,«component.byte03»,«component.byte04», «component.byte05», «component.byte06»};
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
	...
	«FOR device: component.devices»
	incoming«device.deviceType.name»=incomingReadings.«device.deviceType.name»;
	«ENDFOR»
...
void updateDisplay(){
	...
	Serial.print(incomingReadings.«device.name»);
	«ENDFOR»
...
'''
	...
	def getSensorValues(Device device){
			val type = device.deviceType
		switch type{
			MoistureSensor: 	'moistureSensor'
			WaterPumpActuator: 	'waterPumpActuator'
			TemperatureSensor: 	'waterPumpActuator'
...
/*
 * 
 * ««««IF component.devices.forEach[d()=> 2+2]»
«««	'dog'
««««ENDIF»

«««	«FOR device: component.devices.filter(Device)»
«««		'cat'
«««	«ENDFOR»
«««	
